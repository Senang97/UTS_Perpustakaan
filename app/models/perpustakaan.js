...
function perpustakaan() {
	this.get = function(res) {
		connection.acquire(function(err,con) {
			con.query('select * from buku left Join kategori on buku.id_kategori = kategori.id_kategori', function(err,result) {
				con.release ();
				res.send(result);
	...
	this.getByID = function(id_kategori,id_buku,res) {
		connection.acquire(function(err,con) {
			con.query('select * from buku left Join kategori on buku.id_kategori = kategori.id_kategori = ?', id_kategori, function(err,result) {
				con.release();
				res.send(result);
